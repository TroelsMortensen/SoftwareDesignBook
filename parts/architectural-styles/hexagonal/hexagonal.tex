...
At the beginning, we model only application behavior, for example in \texttt{PlaceOrderService} or \texttt{TransferFundsUseCase}. These classes will be responsible for the business logic of the application, i.e. actually executing a use case, with what that entails. At this stage we do not need to commit to \texttt{Spring}, \texttt{ASP.NET}, \texttt{PostgreSQL}, or a message broker. We intentionally start here because architecture decisions become clearer when behavior is visible first.
...
Next, we have the implementation of the port. It is a service class that implements the port interface and contains the business logic for the use case. Here, we just create a new \texttt{Order} and save it to the database, through the output port \texttt{SaveOrderPort}. This latter port/interface is implemented by a driven adapter, and accesses some storage, e.g. a database.

There is also a second output port, \texttt{PublishOrderEventPort}, that is implemented by a driven adapter and publishes an event. This event is picked up, and we can react to it, e.g. send an email to the customer. We will probably need some kind of message broker, or event bus, but I leave these details for later.
...
Obviously, multiple driving adapters can be used to invoke the same use case. For example, we could have a REST controller, a CLI handler, and a GUI controller all invoking the same use case. This is why we need the input port, to abstract the use case from the driving adapter.
...
Let's start with the first output port, \texttt{LoadOrderPort}. It is used to load an order from the database. I use here the \texttt{Optional} \todo{add reference, when Optional pattern is written} pattern to indicate that the order may not be found.
...
And then next port to save an order to the database. You may want to combine the two interfaces into one, with two methods. This is very common, but for this example, we will keep them separate.
...
And then an implementation of the latter port. Here we use the \texttt{Spring Data JPA} library to save the order to the database. You could also use raw SQL, or a ORM like Hibernate.
...
PlaceOrderCommand command = 
                    new PlaceOrderCommand(
                        request.customerId(), 
                        lineMapper.toDomain(request.lines())
                    );
\end{ridercode}
...
\todo{really not what I had in mind, provide AI with example}
