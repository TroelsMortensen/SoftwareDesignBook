\section{Case Study Domain Overview}
\label{sec:case_study_domain_overview}

Throughout this book, we use a single running case study called \textit{Abyssal Watch}. The setting is a near-future world where deep-ocean Breaches are monitored continuously because they can produce large biological anomalies known as Kaiju. The operational challenge is not only to detect anomalies, but to coordinate people, vessels, and sensing infrastructure under uncertain conditions and intermittent connectivity.

The Abyssal Watch initiative operates through Command Carriers that deploy Scout Vessels, collect Observations, and oversee Missions in assigned sectors. Some operations are routine surveys, while others become high-pressure tracking missions when a Kaiju signature emerges. This gives us a domain with clear responsibilities, constrained resources, and safety-critical decisions. As a result, it is a useful foundation for discussing boundaries, interactions, trade-offs, and design decisions across multiple architectural styles.

In practical terms, we will revisit the same domain from different viewpoints: domain modeling, decomposition, data flow, integration, testing strategy, and evolutionary design. Keeping one consistent context helps us focus on architectural reasoning rather than re-learning a new problem in each chapter.

\subsection{Domain at a Glance}
